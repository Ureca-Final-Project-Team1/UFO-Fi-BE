...
public interface MyPageFollowApiSpec {
    ...
    ResponseEntity<ResponseBody<FollowersReadRes>> readFollowers(@RequestParam Long userId);

    @Operation(summary = "내 팔로잉 목록 조회 API", description = "내가 팔로우 하는 유저 목록을 조회한다.")
    @ApiResponse(useReturnTypeSchema = true)
    @GetMapping("/v1/mypage/followings")
    ResponseEntity<ResponseBody<FollowingsReadRes>> readFollowings(@RequestParam Long userId);
}
